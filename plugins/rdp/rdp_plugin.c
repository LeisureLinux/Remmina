 ...
 * https://github.com/FreeRDP/FreeRDP/commit/8c5d96784d and are missing on older
 * FreeRDP, so we add them here. They should be removed from here after all
 * distributed versions of FreeRDP (libwinpr) will have
 * CommandLineParseCommaSeparatedValuesEx() onboard.
 *
 * (C) Copyright goes to the FreeRDP authors.
 */
static char **remmina_rdp_CommandLineParseCommaSeparatedValuesEx(
...
static char **remmina_rdp_CommandLineParseCommaSeparatedValues(const char *list,
                                                               size_t *count) {
    TRACE_CALL(__func__);
    return remmina_rdp_CommandLineParseCommaSeparatedValuesEx(NULL, list,
                                                              count);
}
...
static BOOL rf_process_event_queue(RemminaProtocolWidget *gp) {
    ...
    if (rfi->event_queue == NULL) return True;

    input = rfi->instance->input;

    remminafile = remmina_plugin_service->protocol_plugin_get_file(gp);

    while ((event = (RemminaPluginRdpEvent *)g_async_queue_try_pop(
        ...
        switch (event->type) {
            ...
            case REMMINA_RDP_EVENT_TYPE_SCANCODE_UNICODE:
                /*
                 * TS_UNICODE_KEYBOARD_EVENT RDP message, see
                 * https://msdn.microsoft.com/en-us/library/cc240585.aspx
                 */
                flags =
                    event->key_event.up ? KBD_FLAGS_RELEASE : KBD_FLAGS_DOWN;
                input->UnicodeKeyboardEvent(input, flags,
                                            event->key_event.unicode_code);
                break;
            ...
            case REMMINA_RDP_EVENT_TYPE_CLIPBOARD_SEND_CLIENT_FORMAT_LIST:
                rfi->clipboard.context->ClientFormatList(
                    rfi->clipboard.context,
                    event->clipboard_formatlist.pFormatList);
                free(event->clipboard_formatlist.pFormatList);
                break;

            case REMMINA_RDP_EVENT_TYPE_CLIPBOARD_SEND_CLIENT_FORMAT_DATA_RESPONSE:
                response.msgFlags = (event->clipboard_formatdataresponse.data)
                                        ? CB_RESPONSE_OK
                                        : CB_RESPONSE_FAIL;
                ...
                rfi->clipboard.context->ClientFormatDataResponse(
                    rfi->clipboard.context, &response);
                break;
            ...
            case REMMINA_RDP_EVENT_TYPE_CLIPBOARD_SEND_CLIENT_FORMAT_DATA_REQUEST:
                ...
                gettimeofday(&(rfi->clipboard.clientformatdatarequest_tv),
                             NULL);
                rfi->clipboard.context->ClientFormatDataRequest(
                    rfi->clipboard.context,
                    event->clipboard_formatdatarequest.pFormatDataRequest);
                free(event->clipboard_formatdatarequest.pFormatDataRequest);
                break;

            case REMMINA_RDP_EVENT_TYPE_SEND_MONITOR_LAYOUT:
                if (remmina_plugin_service->file_get_int(remminafile,
                                                         "multimon", FALSE)) {
                    freerdp_settings_set_bool(rfi->settings,
                                              FreeRDP_UseMultimon, TRUE);
                    /* TODO Add an option for this */
                    freerdp_settings_set_bool(rfi->settings,
                                              FreeRDP_ForceMultimon, TRUE);
                    freerdp_settings_set_bool(rfi->settings, FreeRDP_Fullscreen,
                                              TRUE);
                    /* got some crashes with g_malloc0, to be investigated */
                    dcml = calloc(freerdp_settings_get_uint32(
                                      rfi->settings, FreeRDP_MonitorCount),
                                  sizeof(DISPLAY_CONTROL_MONITOR_LAYOUT));
                    REMMINA_PLUGIN_DEBUG(
                        "REMMINA_RDP_EVENT_TYPE_SEND_MONITOR_LAYOUT:");
                    if (!dcml) break;

                    const rdpMonitor *base = freerdp_settings_get_pointer(
                        rfi->settings, FreeRDP_MonitorDefArray);
                    for (gint i = 0;
                         i < freerdp_settings_get_uint32(rfi->settings,
                                                         FreeRDP_MonitorCount);
                         ++i) {
                        const rdpMonitor *current = &base[i];
                        REMMINA_PLUGIN_DEBUG(
                            "Sending display layout for monitor n° %d", i);
                        dcml[i].Flags = (current->is_primary
                                             ? DISPLAY_CONTROL_MONITOR_PRIMARY
                                             : 0);
                        REMMINA_PLUGIN_DEBUG("Monitor %d is primary: %d", i,
                                             dcml[i].Flags);
                        dcml[i].Left = current->x;
                        REMMINA_PLUGIN_DEBUG("Monitor %d x: %d", i,
                                             dcml[i].Left);
                        dcml[i].Top = current->y;
                        REMMINA_PLUGIN_DEBUG("Monitor %d y: %d", i,
                                             dcml[i].Top);
                        dcml[i].Width = current->width;
                        REMMINA_PLUGIN_DEBUG("Monitor %d width: %d", i,
                                             dcml[i].Width);
                        dcml[i].Height = current->height;
                        REMMINA_PLUGIN_DEBUG("Monitor %d height: %d", i,
                                             dcml[i].Height);
                        dcml[i].PhysicalWidth =
                            current->attributes.physicalWidth;
                        REMMINA_PLUGIN_DEBUG("Monitor %d physical width: %d", i,
                                             dcml[i].PhysicalWidth);
                        dcml[i].PhysicalHeight =
                            current->attributes.physicalHeight;
                        REMMINA_PLUGIN_DEBUG("Monitor %d physical height: %d",
                                             i, dcml[i].PhysicalHeight);
                        if (current->attributes.orientation)
                            dcml[i].Orientation =
                                current->attributes.orientation;
                        else
                            dcml[i].Orientation =
                                event->monitor_layout.desktopOrientation;
                        REMMINA_PLUGIN_DEBUG("Monitor %d orientation: %d", i,
                                             dcml[i].Orientation);
                        dcml[i].DesktopScaleFactor =
                            event->monitor_layout.desktopScaleFactor;
                        dcml[i].DeviceScaleFactor =
                            event->monitor_layout.deviceScaleFactor;
                    }
                    rfi->dispcontext->SendMonitorLayout(
                        rfi->dispcontext,
                        freerdp_settings_get_uint32(rfi->settings,
                                                    FreeRDP_MonitorCount),
                        dcml);
                    g_free(dcml);
                } else {
                    ...
                    if (dcml) {
                        ...
                        dcml->Orientation =
                            event->monitor_layout.desktopOrientation;
                        dcml->DesktopScaleFactor =
                            event->monitor_layout.desktopScaleFactor;
                        dcml->DeviceScaleFactor =
                            event->monitor_layout.deviceScaleFactor;
                        rfi->dispcontext->SendMonitorLayout(rfi->dispcontext, 1,
                                                            dcml);
                        g_free(dcml);
                    }
                }
...
static gboolean remmina_rdp_tunnel_init(RemminaProtocolWidget *gp) {
    ...
    REMMINA_PLUGIN_DEBUG("protocol_plugin_start_direct_tunnel() returned %s",
                         hostport);
    ...
    if (!rfi->is_reconnecting) {
        ...
        freerdp_settings_set_string(rfi->settings, FreeRDP_ServerHostname,
                                    host);

        if (cert_port == 3389) {
            freerdp_settings_set_string(rfi->settings, FreeRDP_CertificateName,
                                        cert_host);
        } else {
    ...
    REMMINA_PLUGIN_DEBUG(
        "Tunnel has been optionally initialized. Now connecting to %s:%d", host,
        port);
...
BOOL rf_auto_reconnect(rfContext *rfi) {
    ...
    if ((cval = remmina_plugin_service->pref_get_value(
             "rdp_reconnect_attempts")) != NULL)
        maxattempts = atoi(cval);
    maxattempts = remmina_plugin_service->file_get_int(
        remminafile, "rdp_reconnect_attempts", maxattempts);
    if (maxattempts <= 0)
        maxattempts = freerdp_settings_get_uint32(
            settings, FreeRDP_AutoReconnectMaxRetries);
    ...
    while (TRUE) {
        ...
        if (rfi->reconnect_nattempt++ >= rfi->reconnect_maxattempts) {
            REMMINA_PLUGIN_DEBUG(
                ...
                freerdp_settings_get_string(rfi->settings,
        ...
        /* Wait until 5 secs have elapsed from last reconnect attempt, while
         * checking for rfi->stop_reconnecting_requested */
        while (time(NULL) - treconn < 5) {
            if (rfi->stop_reconnecting_requested) break;
...
BOOL rf_end_paint(rdpContext *context) {
    ...
    if (gdi->primary->hdc->hwnd->ninvalid < 1) return TRUE;
...
static BOOL rf_desktop_resize(rdpContext *context) {
     ...
     * this will also destroy gdi->primary_buffer, making our rfi->surface
     * invalid */
...
static BOOL rf_keyboard_set_indicators(rdpContext *context, UINT16 led_flags) {
    ...
    if (GDK_IS_X11_DISPLAY(disp)) {
        ...
        XkbLockModifiers(x11_display, XkbUseCoreKbd, LockMask | Mod2Mask,
...
BOOL rf_keyboard_set_ime_status(rdpContext *context, UINT16 imeId,
...
static BOOL remmina_rdp_pre_connect(freerdp *instance) {
    ...
    freerdp_settings_set_uint32(settings, FreeRDP_OsMajorType,
                                OSMAJORTYPE_UNIX);
    freerdp_settings_set_uint32(settings, FreeRDP_OsMinorType,
    ...
    if (!freerdp_client_load_addins(channels, settings)) return FALSE;
...
static BOOL remmina_rdp_post_connect(freerdp *instance) {
    ...
    if (instance->context->codecs->h264 == NULL &&
...
static BOOL remmina_rdp_authenticate(freerdp *instance, char **username,
    ...
    ret = remmina_plugin_service->protocol_plugin_init_auth(
        ...
        remmina_plugin_service->file_get_string(remminafile, "domain"), NULL);
    if (ret == GTK_RESPONSE_OK) {
        s_username =
            remmina_plugin_service->protocol_plugin_init_get_username(gp);
        if (s_username)
            freerdp_settings_set_string(rfi->settings, FreeRDP_Username,
                                        s_username);

        s_password =
            remmina_plugin_service->protocol_plugin_init_get_password(gp);
        if (s_password)
            freerdp_settings_set_string(rfi->settings, FreeRDP_Password,
                                        s_password);

        s_domain = remmina_plugin_service->protocol_plugin_init_get_domain(gp);
        if (s_domain)
            freerdp_settings_set_string(rfi->settings, FreeRDP_Domain,
                                        s_domain);

        remmina_plugin_service->file_set_string(remminafile, "username",
                                                s_username);
        remmina_plugin_service->file_set_string(remminafile, "domain",
                                                s_domain);

        save =
            remmina_plugin_service->protocol_plugin_init_get_savepassword(gp);
        if (save) {
            // User has requested to save credentials. We put the password
            // into remminafile->settings. It will be saved later, on successful
            // connection, by rcw.c
...
static BOOL remmina_rdp_gw_authenticate(freerdp *instance, char **username,
                                        char **password, char **domain) {
    TRACE_CALL(__func__);
    gchar *s_username, *s_password, *s_domain;
    gint ret;
    ...
    disablepasswordstoring = remmina_plugin_service->file_get_int(
        remminafile, "disablepasswordstoring", FALSE);
    basecredforgw = remmina_plugin_service->file_get_int(
        remminafile, "base-cred-for-gw", FALSE);
    ...
    if (ret == GTK_RESPONSE_OK) {
        s_username =
            remmina_plugin_service->protocol_plugin_init_get_username(gp);
        if (s_username)
            freerdp_settings_set_string(rfi->settings, FreeRDP_GatewayUsername,
                                        s_username);

        s_password =
            remmina_plugin_service->protocol_plugin_init_get_password(gp);
        if (s_password)
            freerdp_settings_set_string(rfi->settings, FreeRDP_GatewayPassword,
                                        s_password);

        s_domain = remmina_plugin_service->protocol_plugin_init_get_domain(gp);
        if (s_domain)
            freerdp_settings_set_string(rfi->settings, FreeRDP_GatewayDomain,
                                        s_domain);
        ...
        if (basecredforgw) {
            ...
            remmina_plugin_service->file_set_string(
                remminafile, "gateway_username", s_username);
            remmina_plugin_service->file_set_string(remminafile,
                                                    "gateway_domain", s_domain);
            if (save)
                remmina_plugin_service->file_set_string(
                    remminafile, "gateway_password", s_password);
            else
                remmina_plugin_service->file_set_string(
                    remminafile, "gateway_password", NULL);
        }
...
static DWORD remmina_rdp_verify_certificate_ex(
    ...
    status = remmina_plugin_service->protocol_plugin_init_certificate(
        gp, subject, issuer, fingerprint);

    if (status == GTK_RESPONSE_OK) return 1;

    return 0;
}

static DWORD remmina_rdp_verify_certificate(
...
static DWORD remmina_rdp_verify_changed_certificate_ex(
    freerdp *instance, const char *host, UINT16 port, const char *common_name,
    const char *subject, const char *issuer, const char *fingerprint,
    ...
    status = remmina_plugin_service->protocol_plugin_changed_certificate(
        gp, subject, issuer, fingerprint, old_fingerprint);
...
static void remmina_rdp_post_disconnect(freerdp *instance) {
    ...
    /* The remaining cleanup will be continued on main thread by
...
static void remmina_rdp_main_loop(RemminaProtocolWidget *gp) {
    ...
    while (!freerdp_shall_disconnect(rfi->instance)) {
        nCount =
            freerdp_get_event_handles(rfi->instance->context, &handles[0], 64);
        if (rfi->event_handle) handles[nCount++] = rfi->event_handle;
        ...
        if (rfi->event_handle &&
            WaitForSingleObject(rfi->event_handle, 0) == WAIT_OBJECT_0) {
        ...
        /* Check if a processed event called freerdp_abort_connect() and exit if
         * true */
        if (WaitForSingleObject(rfi->instance->context->abortEvent, 0) ==
            WAIT_OBJECT_0)
        ...
        if (!freerdp_check_event_handles(rfi->instance->context)) {
            if (rf_auto_reconnect(rfi)) {
                /* Reset the possible reason/error which made us doing many
                 * reconnection reattempts and continue */
                remmina_plugin_service->protocol_plugin_set_error(gp, NULL);
                continue;
            }
            if (freerdp_get_last_error(rfi->instance->context) ==
...
int remmina_rdp_load_static_channel_addin(rdpChannels *channels,
                                          rdpSettings *settings, char *name,
                                          void *data) {
    TRACE_CALL(__func__);
    ...
    if (entryEx) {
        if (freerdp_channels_client_load_ex(channels, settings, entryEx,
                                            data) == 0) {
            fprintf(stderr, "loading channel %s\n", name);
            return TRUE;
        }
    } else if (entry) {
        if (freerdp_channels_client_load(channels, settings, entry, data) ==
            0) {
            fprintf(stderr, "loading channel %s\n", name);
            return TRUE;
        }
 ...
 *   - For each enumerated local printer tries to set the Printer Name and
 * Driver.
 * @return 1 if there are other printers to scan or 0 when it's done.
 */
int remmina_rdp_set_printers(void *user_data, unsigned flags,
                             cups_dest_t *dest) {
    ...
    /** @warning printer-make-and-model is not always the same as on the
     * Windows, therefore it fails finding to right one and it fails to add the
     * printer.
     *
    ...
    RemminaFile *remminafile =
        remmina_plugin_service->protocol_plugin_get_file(gp);
    const gchar *s = remmina_plugin_service->file_get_string(
        remminafile, "printer_overrides");
    ...
    remmina_rdp_load_static_channel_addin(channels, rfi->settings, "rdpdr",
                                          rfi->settings);
    ...
    if (s) {
        ...
        if (d) {
            ...
            REMMINA_PLUGIN_DEBUG("Printer DriverName set to: %s",
                                 printer->DriverName);
            g_free(d);
        } else {
            /**
    ...
    if (!freerdp_device_collection_add(rfi->settings,
...
static void remmina_rdp_send_ctrlaltdel(RemminaProtocolWidget *gp) {
    ...
    remmina_plugin_service->protocol_plugin_send_keys_signals(
        rfi->drawing_area, keys, G_N_ELEMENTS(keys),
...
static gboolean remmina_rdp_main(RemminaProtocolWidget *gp) {
    ...
    datapath = g_build_path(
        "/", remmina_plugin_service->file_get_user_datadir(), "RDP", NULL);
    REMMINA_PLUGIN_DEBUG("RDP data path is %s", datapath);

    if ((datapath != NULL) && (datapath[0] != '\0'))
        if (access(datapath, W_OK) == 0)
            freerdp_settings_set_string(rfi->settings, FreeRDP_ConfigPath,
                                        datapath);
    g_free(datapath);

#if defined(PROXY_TYPE_IGNORE)
    if (!remmina_plugin_service->file_get_int(remminafile, "useproxyenv", FALSE)
            ? TRUE
            : FALSE) {
        REMMINA_PLUGIN_DEBUG("Not using system proxy settings");
        freerdp_settings_set_uint32(rfi->settings, FreeRDP_ProxyType,
                                    PROXY_TYPE_IGNORE);
    }
#endif

    if (!remmina_rdp_tunnel_init(gp)) return FALSE;

    freerdp_settings_set_bool(rfi->settings, FreeRDP_AutoReconnectionEnabled,
                              (remmina_plugin_service->file_get_int(
                                   remminafile, "disableautoreconnect", FALSE)
                                   ? FALSE
                                   : TRUE));
    /* Disable RDP auto reconnection when SSH tunnel is enabled */
    if (remmina_plugin_service->file_get_int(remminafile, "ssh_tunnel_enabled",
                                             FALSE))
        freerdp_settings_set_bool(rfi->settings,
    ...
    freerdp_settings_set_uint32(
        ...
        remmina_plugin_service->file_get_int(remminafile, "colordepth", 99));
    ...
    if (!gfx_h264_available &&
        (freerdp_settings_get_bool(rfi->settings, FreeRDP_ColorDepth) == 65 ||
         freerdp_settings_get_bool(rfi->settings, FreeRDP_ColorDepth == 66)))
        freerdp_settings_set_uint32(rfi->settings, FreeRDP_ColorDepth,
                                    64);  // Fallback to GFX RFX
    ...
    if (freerdp_settings_get_uint32(rfi->settings, FreeRDP_ColorDepth) == 0) {
    ...
    } else if (freerdp_settings_get_uint32(rfi->settings, FreeRDP_ColorDepth) ==
               63) {
        ...
        freerdp_settings_set_bool(rfi->settings,
                                  FreeRDP_SupportGraphicsPipeline, TRUE);
        freerdp_settings_set_bool(rfi->settings, FreeRDP_GfxH264,
                                  gfx_h264_available);
        freerdp_settings_set_bool(rfi->settings, FreeRDP_GfxAVC444, FALSE);
    } else if (freerdp_settings_get_uint32(rfi->settings, FreeRDP_ColorDepth) ==
               66) {
        ...
        freerdp_settings_set_bool(rfi->settings, FreeRDP_GfxH264,
                                  gfx_h264_available);
        freerdp_settings_set_bool(rfi->settings, FreeRDP_GfxAVC444,
                                  gfx_h264_available);
    } else if (freerdp_settings_get_uint32(rfi->settings, FreeRDP_ColorDepth) ==
               99) {
    ...
    if (freerdp_settings_get_bool(rfi->settings, FreeRDP_RemoteFxCodec) ||
        ...
        freerdp_settings_get_bool(rfi->settings,
    ...
    if (freerdp_settings_get_bool(rfi->settings,
                                  FreeRDP_SupportGraphicsPipeline) &&
        freerdp_settings_get_bool(rfi->settings, FreeRDP_GfxH264)) {
        if (freerdp_settings_get_uint32(rfi->settings, FreeRDP_DesktopWidth) <
            ...
            freerdp_settings_set_uint32(rfi->settings, FreeRDP_DesktopWidth,
                                        AVC_MIN_DESKTOP_WIDTH);
        if (freerdp_settings_get_uint32(rfi->settings, FreeRDP_DesktopHeight) <
            AVC_MIN_DESKTOP_HEIGHT)
            freerdp_settings_set_uint32(rfi->settings, FreeRDP_DesktopHeight,
                                        AVC_MIN_DESKTOP_HEIGHT);
    }

    /* Workaround for FreeRDP issue #5119. This will make our horizontal
     * resolution an even value, but it will add a vertical black 1 pixel line
     * on the right of the desktop */
    if ((freerdp_settings_get_uint32(rfi->settings, FreeRDP_DesktopWidth) &
         1) != 0) {
        UINT32 tmp =
            freerdp_settings_get_uint32(rfi->settings, FreeRDP_DesktopWidth);
        freerdp_settings_set_uint32(rfi->settings, FreeRDP_DesktopWidth,
                                    tmp - 1);
    }

    remmina_plugin_service->protocol_plugin_set_width(
        gp, freerdp_settings_get_uint32(rfi->settings, FreeRDP_DesktopWidth));
    remmina_plugin_service->protocol_plugin_set_height(
        gp, freerdp_settings_get_uint32(rfi->settings, FreeRDP_DesktopHeight));
     ...
     */
    gchar *proxy_type = g_strdup(
        remmina_plugin_service->file_get_string(remminafile, "proxy_type"));
    gchar *proxy_username = g_strdup(
        remmina_plugin_service->file_get_string(remminafile, "proxy_username"));
    gchar *proxy_password = g_strdup(
        remmina_plugin_service->file_get_string(remminafile, "proxy_password"));
    gchar *proxy_hostname = g_strdup(
        remmina_plugin_service->file_get_string(remminafile, "proxy_hostname"));
    gint proxy_port =
        remmina_plugin_service->file_get_int(remminafile, "proxy_port", 80);
    ...
    if (proxy_type && proxy_hostname) {
        if (g_strcmp0(proxy_type, "no_proxy") == 0)
            ...
            g_warning(
                "Invalid proxy protocol, at the moment only no_proxy, HTTP and "
                "SOCKS5 are supported");
        REMMINA_PLUGIN_DEBUG(
            "ProxyType set to: %" PRIu32,
            freerdp_settings_get_uint32(rfi->settings, FreeRDP_ProxyType));
        freerdp_settings_set_string(rfi->settings, FreeRDP_ProxyHostname,
                                    proxy_hostname);
        if (proxy_username)
            freerdp_settings_set_string(rfi->settings, FreeRDP_ProxyUsername,
                                        proxy_username);
        if (proxy_password)
            freerdp_settings_set_string(rfi->settings, FreeRDP_ProxyPassword,
                                        proxy_password);
        if (proxy_port)
            freerdp_settings_set_uint16(rfi->settings, FreeRDP_ProxyPort,
                                        proxy_port);
    }
    ...
    if (remmina_plugin_service->file_get_int(remminafile, "base-cred-for-gw",
                                             FALSE)) {
        // Reset gateway credentials
        remmina_plugin_service->file_set_string(remminafile, "gateway_username",
                                                NULL);
        remmina_plugin_service->file_set_string(remminafile, "gateway_domain",
                                                NULL);
        remmina_plugin_service->file_set_string(remminafile, "gateway_password",
                                                NULL);
    }
    ...
    if (s) {
        ...
        if (remmina_plugin_service->file_get_int(remminafile, "websockets",
                                                 FALSE))
            freerdp_settings_set_bool(rfi->settings,
        ...
        if (g_strcmp0(cs, "http") == 0) {
            ...
            freerdp_settings_set_bool(rfi->settings,
                                      FreeRDP_GatewayHttpTransport, FALSE);
        ...
        remmina_plugin_service->get_server_port(s, 443, &gateway_host,
                                                &gateway_port);
        freerdp_settings_set_string(rfi->settings, FreeRDP_GatewayHostname,
                                    gateway_host);
        freerdp_settings_set_uint32(rfi->settings, FreeRDP_GatewayPort,
                                    gateway_port);
        freerdp_settings_set_bool(rfi->settings, FreeRDP_GatewayEnabled, TRUE);
        freerdp_settings_set_bool(rfi->settings,
                                  FreeRDP_GatewayUseSameCredentials, TRUE);
    }
    /* Remote Desktop Gateway domain */
    if (remmina_plugin_service->file_get_string(remminafile,
                                                "gateway_domain")) {
        freerdp_settings_set_string(rfi->settings, FreeRDP_GatewayDomain,
                                    remmina_plugin_service->file_get_string(
                                        remminafile, "gateway_domain"));
        freerdp_settings_set_bool(rfi->settings,
    ...
    if (remmina_plugin_service->file_get_string(remminafile,
        ...
        freerdp_settings_set_string(rfi->settings, FreeRDP_GatewayUsername,
                                    remmina_plugin_service->file_get_string(
                                        remminafile, "gateway_username"));
        freerdp_settings_set_bool(rfi->settings,
                                  FreeRDP_GatewayUseSameCredentials, FALSE);
    }
    /* Remote Desktop Gateway password */
    s = remmina_plugin_service->file_get_string(remminafile,
                                                "gateway_password");
    if (s) {
    ...
    if (freerdp_settings_get_bool(rfi->settings, FreeRDP_GatewayEnabled) &&
        ...
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_GatewayDomain,
            freerdp_settings_get_string(rfi->settings, FreeRDP_Domain));
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_GatewayUsername,
            freerdp_settings_get_string(rfi->settings, FreeRDP_Username));
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_GatewayPassword,
            freerdp_settings_get_string(rfi->settings, FreeRDP_Password));
    }
    /* Remote Desktop Gateway usage */
    if (freerdp_settings_get_bool(rfi->settings, FreeRDP_GatewayEnabled))
        freerdp_set_gateway_usage_method(
            rfi->settings, remmina_plugin_service->file_get_int(
                               remminafile, "gateway_usage", FALSE)
                               ? TSC_PROXY_MODE_DETECT
                               : TSC_PROXY_MODE_DIRECT);

    freerdp_settings_set_string(rfi->settings, FreeRDP_GatewayAccessToken,
                                remmina_plugin_service->file_get_string(
                                    remminafile, "gatewayaccesstoken"));

    freerdp_settings_set_uint32(
        rfi->settings, FreeRDP_AuthenticationLevel,
        remmina_plugin_service->file_get_int(
    ...
    freerdp_settings_set_bool(
        rfi->settings, FreeRDP_IgnoreCertificate,
        remmina_plugin_service->file_get_int(remminafile, "cert_ignore", 0));
    freerdp_settings_set_bool(
        rfi->settings, FreeRDP_OldLicenseBehaviour,
        remmina_plugin_service->file_get_int(remminafile, "old-license", 0));
    freerdp_settings_set_bool(rfi->settings,
                              FreeRDP_AllowUnanouncedOrdersFromServer,
                              remmina_plugin_service->file_get_int(
                                  remminafile, "relax-order-checks", 0));
    freerdp_settings_set_uint32(
        rfi->settings, FreeRDP_GlyphSupportLevel,
        (remmina_plugin_service->file_get_int(remminafile, "glyph-cache", 0)
             ? GLYPH_SUPPORT_FULL
             : GLYPH_SUPPORT_NONE));

    if ((cs = remmina_plugin_service->file_get_string(remminafile,
                                                      "clientname")))
        freerdp_settings_set_string(rfi->settings, FreeRDP_ClientHostname, cs);
    else
        freerdp_settings_set_string(rfi->settings, FreeRDP_ClientHostname,
                                    g_get_host_name());

    /* Client Build number is optional, if not specified defaults to 0, allow
     * for comments to appear after number */
    if ((cs = remmina_plugin_service->file_get_string(remminafile,
                                                      "clientbuild"))) {
        if (*cs) {
            UINT32 val = strtoul(cs, NULL, 0);
            freerdp_settings_set_uint32(rfi->settings, FreeRDP_ClientBuild,
                                        val);
        }
    }
    ...
    if (remmina_plugin_service->file_get_string(remminafile,
                                                "loadbalanceinfo")) {
        char *tmp = strdup(remmina_plugin_service->file_get_string(
            remminafile, "loadbalanceinfo"));
        rfi->settings->LoadBalanceInfo = (BYTE *)tmp;

        freerdp_settings_set_uint32(rfi->settings,
                                    FreeRDP_LoadBalanceInfoLength, strlen(tmp));
    }

    if (remmina_plugin_service->file_get_string(remminafile, "exec"))
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_AlternateShell,
            remmina_plugin_service->file_get_string(remminafile, "exec"));

    if (remmina_plugin_service->file_get_string(remminafile, "execpath"))
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_ShellWorkingDirectory,
            remmina_plugin_service->file_get_string(remminafile, "execpath"));

    sm = g_strdup_printf("rdp_quality_%i",
                         remmina_plugin_service->file_get_int(
                             remminafile, "quality", DEFAULT_QUALITY_0));
    value = remmina_plugin_service->pref_get_value(sm);
    g_free(sm);

    if (value && value[0]) {
        freerdp_settings_set_uint32(rfi->settings, FreeRDP_PerformanceFlags,
                                    strtoul(value, NULL, 16));
    } else {
        switch (remmina_plugin_service->file_get_int(remminafile, "quality",
                                                     DEFAULT_QUALITY_0)) {
            case 9:
                freerdp_settings_set_uint32(
                    rfi->settings, FreeRDP_PerformanceFlags, DEFAULT_QUALITY_9);
                break;

            case 2:
                freerdp_settings_set_uint32(
                    rfi->settings, FreeRDP_PerformanceFlags, DEFAULT_QUALITY_2);
                break;

            case 1:
                freerdp_settings_set_uint32(
                    rfi->settings, FreeRDP_PerformanceFlags, DEFAULT_QUALITY_1);
                break;

            case 0:
            default:
                freerdp_settings_set_uint32(
                    rfi->settings, FreeRDP_PerformanceFlags, DEFAULT_QUALITY_0);
                break;
        }
    }
    g_free(value);
    ...
    if ((cs =
             remmina_plugin_service->file_get_string(remminafile, "network"))) {
        guint32 type = 0;
     ...
     * like freerdp_settings_set_bool(rfi->settings, FreeRDP_DisableWallpaper,
     * freerdp_settings_set_bool(rfi->settings, FreeRDP_AllowFontSmoothing… or
     * freerdp_get_param_bool() function will return the wrong value
     */
    freerdp_performance_flags_split(rfi->settings);

#if FREERDP_CHECK_VERSION(2, 3, 0)
    freerdp_settings_set_string(
        rfi->settings, FreeRDP_KeyboardRemappingList,
        remmina_plugin_service->pref_get_value("rdp_kbd_remap"));
    REMMINA_PLUGIN_DEBUG("rdp_keyboard_remapping_list: %s",
                         rfi->settings->KeyboardRemappingList);
#endif
    freerdp_settings_set_uint32(rfi->settings, FreeRDP_KeyboardLayout,
    ...
    if (remmina_plugin_service->file_get_int(remminafile, "restricted-admin",
                                             FALSE)) {
        freerdp_settings_set_bool(rfi->settings, FreeRDP_ConsoleSession, TRUE);
        freerdp_settings_set_bool(rfi->settings,
                                  FreeRDP_RestrictedAdminModeRequired, TRUE);
    }
    ...
    if (remmina_plugin_service->file_get_string(remminafile, "pth")) {
        ...
        freerdp_settings_set_string(
            ...
            remmina_plugin_service->file_get_string(remminafile, "pth"));
        remmina_plugin_service->file_set_int(remminafile, "restricted-admin",
                                             TRUE);
    }
    ...
    if (g_strcmp0(cs, "rdp") == 0) {
        ...
        freerdp_settings_set_bool(rfi->settings, FreeRDP_UseRdpSecurityLayer,
                                  TRUE);
        ...
        freerdp_settings_set_bool(rfi->settings, FreeRDP_NegotiateSecurityLayer,
                                  TRUE);
    }

    freerdp_settings_set_bool(rfi->settings, FreeRDP_CompressionEnabled, TRUE);
    if (remmina_plugin_service->file_get_int(remminafile, "disable_fastpath",
                                             FALSE)) {
    ...
    freerdp_settings_set_uint16(rfi->settings, FreeRDP_DesktopOrientation,
                                desktopOrientation);
    if (desktopScaleFactor != 0 && deviceScaleFactor != 0) {
        freerdp_settings_set_uint32(rfi->settings, FreeRDP_DesktopScaleFactor,
                                    desktopScaleFactor);
        freerdp_settings_set_uint32(rfi->settings, FreeRDP_DeviceScaleFactor,
                                    deviceScaleFactor);
    }
    ...
    freerdp_settings_set_bool(rfi->settings, FreeRDP_SupportDisplayControl,
                              TRUE);
    if (freerdp_settings_get_bool(rfi->settings,
    ...
    if (g_strcmp0(cs, "remote") == 0) {
        freerdp_settings_set_bool(rfi->settings, FreeRDP_RemoteConsoleAudio,
                                  TRUE);
    ...
    if (cs != NULL && cs[0] != '\0') {
        if (g_strcmp0(cs, "0") == 0) {
            ...
            freerdp_settings_set_bool(rfi->settings, FreeRDP_AudioCapture,
                                      TRUE);
            REMMINA_PLUGIN_DEBUG("“microphone” set to “%s”", cs);
            char **p;
            size_t count;

            p = remmina_rdp_CommandLineParseCommaSeparatedValuesEx(
                "audin", g_strdup(cs), &count);
    ...
    cs = remmina_plugin_service->file_get_string(remminafile,
                                                 "freerdp_log_level");
    if (cs != NULL && cs[0] != '\0')
    ...
    if (cs != NULL && cs[0] != '\0') {
        char **p;
        size_t count;
        p = remmina_rdp_CommandLineParseCommaSeparatedValues(g_strdup(cs),
                                                             &count);
    ...
    if (cs != NULL && cs[0] != '\0') {
        ...
        remmina_rdp_load_static_channel_addin(
            channels, rfi->settings, "rdp2tcp", rfi->settings->RDP2TCPArgs);
    }
    ...
    if (cs != NULL && cs[0] != '\0') {
        ...
        if (val > 600000 || val <= 0) val = 600000;
        freerdp_settings_set_uint32(rfi->settings, FreeRDP_TcpAckTimeout,
                                    (UINT32)val);
    }
#endif

    if (remmina_plugin_service->file_get_int(remminafile, "preferipv6", FALSE)
            ? TRUE
            : FALSE)
        freerdp_settings_set_bool(rfi->settings, FreeRDP_PreferIPv6OverIPv4,
                                  TRUE);

    freerdp_settings_set_bool(rfi->settings, FreeRDP_RedirectClipboard,
                              remmina_plugin_service->file_get_int(
                                  remminafile, "disableclipboard", FALSE)
                                  ? FALSE
                                  : TRUE);
    ...
    if (cs != NULL && cs[0] != '\0') {
        ...
        for (i = 0; folders[i] != NULL; i++) {
            ...
            p = remmina_rdp_CommandLineParseCommaSeparatedValuesEx(
                "drive", g_strdup(folders[i]), &count);
            status = freerdp_client_add_device_channel(rfi->settings, count, p);
            g_free(p);
        }
        g_strfreev(folders);
    ...
    if (remmina_plugin_service->file_get_int(remminafile, "shareprinter",
                                             FALSE)) {
#ifdef HAVE_CUPS
        REMMINA_PLUGIN_DEBUG("Sharing printers");
        if (cupsEnumDests(CUPS_DEST_FLAGS_NONE, 1000, NULL, 0, 0,
                          remmina_rdp_set_printers, rfi))
    ...
    if (remmina_plugin_service->file_get_int(remminafile, "multimon", FALSE)) {
        ...
        gchar *monitorids_string = g_strdup(
            remmina_plugin_service->file_get_string(remminafile, "monitorids"));
        ...
        if (monitorids_string != NULL && monitorids_string[0] != '\0') {
            if (g_strstr_len(monitorids_string, -1, ",") != NULL) {
                if (g_strstr_len(monitorids_string, -1, ":") != NULL) {
                    rdpMonitor *base =
                        (rdpMonitor *)freerdp_settings_get_pointer(
                            rfi->settings, FreeRDP_MonitorDefArray);
                    ...
                    for (i = 0; i < g_strv_length(temp_items); i++) {
                        ...
                        if (i == 0)
                            ...
                            monitorids = g_strdup_printf("%s,%s", monitorids,
                                                         rot_items[0]);
                        current->attributes.orientation = atoi(rot_items[1]);
                        REMMINA_PLUGIN_DEBUG("Monitor n %d orientation: %d", i,
                                             current->attributes.orientation);
                    }
                } else {
        ...
        if (monitorids != NULL && monitorids[0] != '\0') {
            UINT32 *base = (UINT32 *)freerdp_settings_get_pointer(
                rfi->settings, FreeRDP_MonitorIds);
            gchar **items;
            items = g_strsplit(monitorids, ",", -1);
            freerdp_settings_set_uint32(rfi->settings, FreeRDP_NumMonitorIds,
                                        g_strv_length(items));
            REMMINA_PLUGIN_DEBUG("NumMonitorIds: %d",
                                 freerdp_settings_get_uint32(
                                     rfi->settings, FreeRDP_NumMonitorIds));
            for (i = 0; i < g_strv_length(items); i++) {
                UINT32 *current = &base[i];
                *current = atoi(items[i]);
                REMMINA_PLUGIN_DEBUG("Added monitor with ID %" PRIu32,
                                     *current);
            }
            g_free(monitorids);
            g_strfreev(items);
        ...
        if (maxwidth && maxheight) {
            REMMINA_PLUGIN_DEBUG(
                "Setting DesktopWidth and DesktopHeight to: %dx%d", maxwidth,
                maxheight);
            freerdp_settings_set_uint32(rfi->settings, FreeRDP_DesktopWidth,
                                        maxwidth);
            freerdp_settings_set_uint32(rfi->settings, FreeRDP_DesktopHeight,
                                        maxheight);
            REMMINA_PLUGIN_DEBUG("DesktopWidth and DesktopHeight set to: %dx%d",
                                 freerdp_settings_get_uint32(
                                     rfi->settings, FreeRDP_DesktopWidth),
                                 freerdp_settings_get_uint32(
                                     rfi->settings, FreeRDP_DesktopHeight));
        } else {
            REMMINA_PLUGIN_DEBUG(
                "Cannot set Desktop Size, we are using the previously set "
                "values: %dx%d",
                freerdp_settings_get_uint32(rfi->settings,
        ...
        remmina_plugin_service->protocol_plugin_set_height(
            gp,
            freerdp_settings_get_uint32(rfi->settings, FreeRDP_DesktopHeight));
    }

    /*if (remmina_plugin_service->file_get_int(remminafile, "sharesmartcard",
     * FALSE)) {*/
    ...
    /*freerdp_settings_set_bool(rfi->settings, FreeRDP_DeviceRedirection,
     * TRUE);*/
    /*remmina_rdp_load_static_channel_addin(channels, rfi->settings, "rdpdr",
     * rfi->settings);*/

    /*const gchar *sn = remmina_plugin_service->file_get_string(remminafile,
     * "smartcardname");*/
    /*if (sn != NULL && sn[0] != '\0')*/
    /*smartcard->Name = _strdup(sn);*/

    /*freerdp_settings_set_bool(rfi->settings, FreeRDP_RedirectSmartCards,
     * TRUE);*/

    /*freerdp_device_collection_add(rfi->settings, (RDPDR_DEVICE *)smartcard);*/
    /*}*/

    if (remmina_plugin_service->file_get_int(remminafile, "passwordispin",
                                             FALSE))
        /* Option works only combined with Username and Domain, because FreeRDP
         * doesn’t know anything about info on smart card */
        freerdp_settings_set_bool(rfi->settings, FreeRDP_PasswordIsSmartcardPin,
                                  TRUE);

    /* /serial[:<name>[,<path>[,<driver>[,permissive]]]] */
    /*if (remmina_plugin_service->file_get_int(remminafile, "shareserial",
     * FALSE)) {*/
    /*RDPDR_SERIAL *serial;*/
    ...
    /*const gchar *sd = remmina_plugin_service->file_get_string(remminafile,
     * "serialdriver");*/
    /*if (sd != NULL && sd[0] != '\0')*/
    /*serial->Driver = _strdup(sd);*/

    /*const gchar *sp = remmina_plugin_service->file_get_string(remminafile,
     * "serialpath");*/
    /*if (sp != NULL && sp[0] != '\0')*/
    /*serial->Path = _strdup(sp);*/

    /*if (remmina_plugin_service->file_get_int(remminafile, "serialpermissive",
     * FALSE))*/
    /*serial->Permissive = _strdup("permissive");*/

    /*freerdp_settings_set_bool(rfi->settings, FreeRDP_RedirectSerialPorts,
     * TRUE);*/

    /*freerdp_device_collection_add(rfi->settings, (RDPDR_DEVICE *)serial);*/
    /*}*/

    /*if (remmina_plugin_service->file_get_int(remminafile, "shareparallel",
     * FALSE)) {*/
    ...
    /*freerdp_settings_set_bool(rfi->settings, FreeRDP_RedirectParallelPorts,
     * TRUE);*/

    /*const gchar *pn = remmina_plugin_service->file_get_string(remminafile,
     * "parallelname");*/
    /*if (pn != NULL && pn[0] != '\0')*/
    /*parallel->Name = _strdup(pn);*/
    /*const gchar *dp = remmina_plugin_service->file_get_string(remminafile,
     * "parallelpath");*/
    /*if (dp != NULL && dp[0] != '\0')*/
     ...
     */
    if (remmina_plugin_service->file_get_int(remminafile, "multitransport",
                                             FALSE)) {
        freerdp_settings_set_bool(rfi->settings, FreeRDP_DeviceRedirection,
                                  TRUE);
        freerdp_settings_set_bool(rfi->settings, FreeRDP_SupportMultitransport,
                                  TRUE);
        freerdp_settings_set_uint32(
            rfi->settings, FreeRDP_MultitransportFlags,
    ...
    /* If needed, force interactive authentication by deleting all
     *authentication fields, forcing libfreerdp to call our callbacks for
     *authentication.
     *	  This usually happens from a second attempt of connection, never on the
     *1st one. */
    ...
    if (!freerdp_connect(rfi->instance)) {
        ...
        if (!orphaned) {
            ...
            switch (e) {
                ...
                case STATUS_LOGON_FAILURE:  // wrong return code from FreeRDP
                                            // introduced at the end of July
                                            // 2016? (fixed with b86c0ba)
                ...
                case FREERDP_ERROR_CONNECT_FAILED:
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp, _("Lost connection to the RDP server “%s”."),
                        freerdp_settings_get_string(rfi->settings,
                                                    FreeRDP_ServerHostname));
                    break;
                case FREERDP_ERROR_DNS_NAME_NOT_FOUND:
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _("Could not find the address for the RDP server "
                          "“%s”."),
                        freerdp_settings_get_string(rfi->settings,
                                                    FreeRDP_ServerHostname));
                    break;
                case FREERDP_ERROR_TLS_CONNECT_FAILED:
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _("Could not connect to the RDP server “%s” via TLS. "
                          "Check that client and server support a common TLS "
                          "version."),
                        freerdp_settings_get_string(rfi->settings,
                                                    FreeRDP_ServerHostname));
                    break;
                case FREERDP_ERROR_SECURITY_NEGO_CONNECT_FAILED:
                    // TRANSLATORS: the placeholder may be either an IP/FQDN or
                    // a server hostname
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _("Unable to establish a connection to the RDP server "
                          "“%s”. Check “Security protocol negotiation”."),
                        freerdp_settings_get_string(rfi->settings,
                                                    FreeRDP_ServerHostname));
                    break;
#ifdef FREERDP_ERROR_POST_CONNECT_FAILED
                case FREERDP_ERROR_POST_CONNECT_FAILED:
                    /* remmina_rdp_post_connect() returned FALSE to libfreerdp.
                     * We saved the error on rfi->postconnect_error */
                    switch (rfi->postconnect_error) {
                        case REMMINA_POSTCONNECT_ERROR_OK:
                            /* We should never come here */
                            remmina_plugin_service->protocol_plugin_set_error(
                                gp, _("Cannot connect to the RDP server “%s”."),
                                freerdp_settings_get_string(
                                    rfi->settings, FreeRDP_ServerHostname));
                            break;
                        case REMMINA_POSTCONNECT_ERROR_GDI_INIT:
                            remmina_plugin_service->protocol_plugin_set_error(
                                gp, _("Could not start libfreerdp-gdi."));
                            break;
                        case REMMINA_POSTCONNECT_ERROR_NO_H264:
                            remmina_plugin_service->protocol_plugin_set_error(
                                gp,
                                _("You requested a H.264 GFX mode for the "
                                  "server “%s”, but your libfreerdp does not "
                                  "support H.264. Please use a non-AVC colour "
                                  "depth setting."),
                                freerdp_settings_get_string(
                                    rfi->settings, FreeRDP_ServerHostname));
                            break;
                ...
                case FREERDP_ERROR_SERVER_DENIED_CONNECTION:
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp, _("The “%s” server refused the connection."),
                        freerdp_settings_get_string(rfi->settings,
                                                    FreeRDP_ServerHostname));
                    break;
#endif
                case 0x800759DB:
                    ...
                    // https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-tsgu/84cd92e4-592c-4219-95d8-18021ac654b0
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _("The Remote Desktop Gateway “%s” denied the user "
                          "“%s\\%s” access due to policy."),
                        freerdp_settings_get_string(rfi->settings,
                ...
                default:
                    ...
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp, _("Cannot connect to the “%s” RDP server."),
                        freerdp_settings_get_string(rfi->settings,
                                                    FreeRDP_ServerHostname));
                    break;
            }
    ...
    if (GET_PLUGIN_DATA(rfi->protocol_widget) == NULL)
        orphaned = True;
    else
        orphaned = False;
    if (!orphaned &&
        freerdp_get_last_error(rfi->instance->context) ==
            FREERDP_ERROR_SUCCESS &&
        !rfi->user_cancelled)
...
static void rfi_uninit(rfContext *rfi) {
    ...
    if (instance) {
        ...
        if (pEntryPoints) IFCALL(pEntryPoints->GlobalUninit);
        free(instance->pClientEntryPoints);
        freerdp_context_free(instance); /* context is rfContext* rfi */
        freerdp_free(instance); /* This implicitly frees instance->context and
                                   rfi is no longer valid */
    }
}
...
static gboolean complete_cleanup_on_main_thread(gpointer data) {
    ...
    /* Notify the RemminaProtocolWidget that we closed our connection, and the
     * GUI interface can be removed */
    if (!orphaned)
...
static gpointer remmina_rdp_main_thread(gpointer data) {
    ...
    do remmina_rdp_main(gp);
    while (!remmina_plugin_service->protocol_plugin_has_error(gp) &&
           rfi->attempt_interactive_authentication == TRUE &&
           !rfi->user_cancelled);
...
static void remmina_rdp_init(RemminaProtocolWidget *gp) {
    ...
    // instance->VerifyChangedCertificate =
...
static gboolean remmina_rdp_open_connection(RemminaProtocolWidget *gp) {
    ...
    rfi->scale =
        remmina_plugin_service->remmina_protocol_widget_get_current_scale_mode(
            gp);

    remminafile = remmina_plugin_service->protocol_plugin_get_file(gp);

    if (pthread_create(&rfi->remmina_plugin_thread, NULL,
                       remmina_rdp_main_thread, gp)) {
        remmina_plugin_service->protocol_plugin_set_error(
            gp, "%s", "Could not start pthread.");
...
static gboolean remmina_rdp_close_connection(RemminaProtocolWidget *gp) {
    ...
    if (!remmina_plugin_service->is_main_thread())
        g_warning(
            "WARNING: %s called on a subthread, which may not work or crash "
            "Remmina.",
            __func__);

    if (rfi && !rfi->connected) {
        /* libfreerdp is attempting to connect, we cannot interrupt our main
         * thread in the connect phase. So we remove "plugin-data" from gp, so
         * our rfi remains "orphan"
         */
...
static gboolean remmina_rdp_query_feature(
    RemminaProtocolWidget *gp, const RemminaProtocolFeature *feature) {
    TRACE_CALL(__func__);
    return TRUE;
}
    ...
    switch (feature->id) {
        ...
        case REMMINA_RDP_FEATURE_SCALE:
            if (rfi) {
                ...
                REMMINA_PLUGIN_DEBUG(
                    "Remmina RDP plugin warning: Null value for rfi by "
...
static void remmina_rdp_keystroke(RemminaProtocolWidget *gp,
                                  const guint keystrokes[], const gint keylen) {
    TRACE_CALL(__func__);
    rfContext *rfi = GET_PLUGIN_DATA(gp);
...
static gboolean remmina_rdp_get_screenshot(RemminaProtocolWidget *gp,
    ...
    /** @todo we should lock FreeRDP subthread to update rfi->primary_buffer,
     * rfi->gdi and w/h, from here to memcpy, but… how ? */
...
static gpointer log_level[] = {"INFO",  "INFO",  "FATAL", "FATAL", "ERROR",
                               "ERROR", "WARN",  "WARN",  "DEBUG", "DEBUG",
                               "TRACE", "TRACE", "OFF",   "OFF",   NULL};

/* Array of key/value pairs for quality selection */
static gpointer quality_list[] = {
    "0", N_("Poor (fastest)"), "1", N_("Medium"), "2", N_("Good"),
    "9", N_("Best (slowest)"), NULL};
...
static gpointer sound_list[] = {"off",    N_("Off"),    "local", N_("Local"),
                                "remote", N_("Remote"), NULL};

/* Array of key/value pairs for security */
static gpointer security_list[] = {"",    N_("Automatic negotiation"),
...
static gpointer gwtransp_list[] = {"http", "HTTP", "rpc", "RPC",
                                   "auto", "Auto", NULL};
...
static gchar clientbuild_tooltip[] =
    N_("Used i.a. by terminal services in a smart card channel to distinguish "
       "client capabilities:\n"
 ...
 * e) Values for REMMINA_PROTOCOL_SETTING_TYPE_SELECT or
 *REMMINA_PROTOCOL_SETTING_TYPE_COMBO f) Setting tooltip g) Validation data
 *pointer, will be passed to the validation callback method. h) Validation
 *callback method (Can be NULL. Every entry will be valid then.) use following
 *prototype: gboolean mysetting_validator_method(gpointer key, gpointer value,
...
static const RemminaProtocolSetting remmina_rdp_basic_settings[] = {
    {REMMINA_PROTOCOL_SETTING_TYPE_SERVER, "server", NULL, FALSE, NULL, NULL,
     NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "username", N_("Username"), FALSE,
     NULL, NULL, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_PASSWORD, "password", N_("Password"), FALSE,
     NULL, NULL, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "domain", N_("Domain"), FALSE, NULL,
     NULL, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_FOLDER, "sharefolder", N_("Share folder"),
     FALSE, NULL,
     N_("Use “Redirect directory” in the advanced tab for multiple "
        "directories"),
     NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "restricted-admin",
     N_("Restricted admin mode"), FALSE, NULL, NULL, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "pth", N_("Password hash"), FALSE,
     NULL, N_("Restricted admin mode password hash"), NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "left-handed",
     N_("Left-handed mouse support"), TRUE, NULL,
     N_("Swap left and right mouse buttons for left-handed mouse support"),
     NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "disable-smooth-scrolling",
     N_("Disable smooth scrolling"), TRUE, NULL, NULL, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "multimon",
     N_("Enable multi monitor"), TRUE, NULL, NULL, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "span",
     N_("Span screen over multiple monitors"), TRUE, NULL, NULL, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "monitorids", N_("List monitor IDs"),
     FALSE, NULL, monitorids_tooltip, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_RESOLUTION, "resolution", NULL, FALSE, NULL,
     NULL, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_SELECT, "colordepth", N_("Colour depth"),
     FALSE, colordepth_list, NULL, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_SELECT, "network",
     N_("Network connection type"), FALSE, network_list, network_tooltip, NULL,
     NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_END, NULL, NULL, FALSE, NULL, NULL, NULL,
     NULL}};
 ...
 * d) Compact disposition
 * e) Values for REMMINA_PROTOCOL_SETTING_TYPE_SELECT or
 * REMMINA_PROTOCOL_SETTING_TYPE_COMBO f) Setting Tooltip
 */
static const RemminaProtocolSetting remmina_rdp_advanced_settings[] = {
    {REMMINA_PROTOCOL_SETTING_TYPE_SELECT, "quality", N_("Quality"), FALSE,
     quality_list, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_SELECT, "security",
     N_("Security protocol negotiation"), FALSE, security_list, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_SELECT, "gwtransp",
     N_("Gateway transport type"), FALSE, gwtransp_list, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_SELECT, "freerdp_log_level",
     N_("FreeRDP log level"), FALSE, log_level, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "freerdp_log_filters",
     N_("FreeRDP log filters"), FALSE, NULL, N_("tag:level[,tag:level[,…]]")},
    {REMMINA_PROTOCOL_SETTING_TYPE_SELECT, "sound", N_("Audio output mode"),
     FALSE, sound_list, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "audio-output",
     N_("Redirect local audio output"), TRUE, NULL, audio_tooltip},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "microphone",
     N_("Redirect local microphone"), TRUE, NULL, microphone_tooltip},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "timeout",
     N_("Connection timeout in ms"), TRUE, NULL, timeout_tooltip},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "gateway_server",
     N_("Remote Desktop Gateway server"), FALSE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "gateway_username",
     N_("Remote Desktop Gateway username"), FALSE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_PASSWORD, "gateway_password",
     N_("Remote Desktop Gateway password"), FALSE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "gateway_domain",
     N_("Remote Desktop Gateway domain"), FALSE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "drive", N_("Redirect directory"),
     FALSE, NULL, drive_tooltip},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "clientname", N_("Client name"), FALSE,
     NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_COMBO, "clientbuild", N_("Client build"),
     FALSE, clientbuild_list, clientbuild_tooltip},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "exec", N_("Start-up program"), FALSE,
     NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "execpath", N_("Start-up path"), FALSE,
     NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "loadbalanceinfo",
     N_("Load balance info"), FALSE, NULL, NULL},
    // TRANSLATORS: Do not use typographic quotation marks, these must stay as
    // "double quote", also know as “Typewriter ("programmer's") quote,
    // ambidextrous.”
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "printer_overrides",
     N_("Override printer drivers"), FALSE, NULL,
     N_("\"Samsung_CLX-3300_Series\":\"Samsung CLX-3300 Series PS\";\"Canon "
        "MF410\":\"Canon MF410 Series UFR II\"")},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "usb", N_("USB device redirection"),
     TRUE, NULL, usb_tooltip},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "serialname", N_("Local serial name"),
     FALSE, NULL, N_("COM1, COM2, etc.")},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "serialdriver",
     N_("Local serial driver"), FALSE, NULL, N_("Serial")},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "serialpath", N_("Local serial path"),
     FALSE, NULL, N_("/dev/ttyS0, /dev/ttyS1, etc.")},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "parallelname",
     N_("Local parallel name"), FALSE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "parallelpath",
     N_("Local parallel device"), FALSE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "smartcardname",
     N_("Name of smart card"), FALSE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "dvc", N_("Dynamic virtual channel"),
     FALSE, NULL, N_("<channel>[,<options>]")},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "vc", N_("Static virtual channel"),
     FALSE, NULL, N_("<channel>[,<options>]")},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "rdp2tcp", N_("TCP redirection"),
     FALSE, NULL, N_("/PATH/TO/rdp2tcp")},
    {REMMINA_PROTOCOL_SETTING_TYPE_TEXT, "rdp_reconnect_attempts",
     N_("Reconnect attempts number"), FALSE, NULL,
     N_("The maximum number of reconnect attempts upon an RDP disconnect "
        "(default: 20)")},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "preferipv6",
     N_("Prefer IPv6 AAAA record over IPv4 A record"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "shareprinter", N_("Share printers"),
     TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "shareserial",
     N_("Share serial ports"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "serialpermissive",
     N_("(SELinux) permissive mode for serial ports"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "shareparallel",
     N_("Share parallel ports"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "sharesmartcard",
     N_("Share a smart card"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "disableclipboard",
     N_("Turn off clipboard sync"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "cert_ignore",
     N_("Ignore certificate"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "old-license",
     N_("Use the old license workflow"), TRUE, NULL,
     N_("It disables CAL and hwId is set to 0")},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "disablepasswordstoring",
     N_("Forget passwords after use"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "console",
     N_("Attach to console (2003/2003 R2)"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "disable_fastpath",
     N_("Turn off fast-path"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "gateway_usage",
     N_("Server detection using Remote Desktop Gateway"), TRUE, NULL, NULL},
#if defined(PROXY_TYPE_IGNORE)
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "useproxyenv",
     N_("Use system proxy settings"), TRUE, NULL, NULL},
#endif
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "disableautoreconnect",
     N_("Turn off automatic reconnection"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "relax-order-checks",
     N_("Relax order checks"), TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "glyph-cache", N_("Glyph cache"),
     TRUE, NULL, NULL},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "multitransport",
     N_("Enable multitransport protocol (UDP)"), TRUE, NULL,
     N_("Using the UDP protocol may improve performance")},
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "base-cred-for-gw",
     N_("Use base credentials for gateway too"), TRUE, NULL, NULL},
#if FREERDP_CHECK_VERSION(2, 3, 1)
    {REMMINA_PROTOCOL_SETTING_TYPE_CHECK, "websockets",
     N_("Enable Gateway websockets support"), TRUE, NULL, NULL},
#endif
...
static const RemminaProtocolFeature remmina_rdp_features[] = {
    {REMMINA_PROTOCOL_FEATURE_TYPE_TOOL, REMMINA_RDP_FEATURE_TOOL_REFRESH,
     N_("Refresh"), NULL, NULL},
    {REMMINA_PROTOCOL_FEATURE_TYPE_SCALE, REMMINA_RDP_FEATURE_SCALE, NULL, NULL,
     NULL},
    {REMMINA_PROTOCOL_FEATURE_TYPE_DYNRESUPDATE,
     REMMINA_RDP_FEATURE_DYNRESUPDATE, NULL, NULL, NULL},
    {REMMINA_PROTOCOL_FEATURE_TYPE_MULTIMON, REMMINA_RDP_FEATURE_MULTIMON, NULL,
     NULL, NULL},
    {REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
     REMMINA_RDP_FEATURE_TOOL_SENDCTRLALTDEL, N_("Send Ctrl+Alt+Delete"), NULL,
     NULL},
    {REMMINA_PROTOCOL_FEATURE_TYPE_UNFOCUS, REMMINA_RDP_FEATURE_UNFOCUS, NULL,
     NULL, NULL},
...
G_MODULE_EXPORT gboolean remmina_plugin_entry(RemminaPluginService *service) {
    ...
    if (vermaj < FREERDP_REQUIRED_MAJOR ||
        (vermaj == FREERDP_REQUIRED_MAJOR &&
         (vermin < FREERDP_REQUIRED_MINOR ||
          (vermin == FREERDP_REQUIRED_MINOR &&
           verrev < FREERDP_REQUIRED_REVISION)))) {
        g_printf(
            "Upgrade your FreeRDP library version from %d.%d.%d to at least "
            "libfreerdp %d.%d.%d "
            "to run the RDP plugin.\n",
            vermaj, vermin, verrev, FREERDP_REQUIRED_MAJOR,
            FREERDP_REQUIRED_MINOR, FREERDP_REQUIRED_REVISION);
        return FALSE;
    }
    ...
    if (!service->register_plugin((RemminaPlugin *)&remmina_rdp)) return FALSE;

    remmina_rdpf.export_hints =
        _("Export connection in Windows .rdp file format");

    if (!service->register_plugin((RemminaPlugin *)&remmina_rdpf)) return FALSE;

    if (!service->register_plugin((RemminaPlugin *)&remmina_rdps)) return FALSE;
    ...
    snprintf(remmina_plugin_rdp_version, sizeof(remmina_plugin_rdp_version),
             "RDP plugin: %s (Git %s), Compiled with libfreerdp %s (%s), "
             "Running with libfreerdp %s (rev %s), H.264 %s",
